		...
		\subsection{Implicitly Defined Structure}
			...
			\subsubsection{Low-Rank Structure}
				...
				\begin{algorithm}[!htbp]
					...
					\myinput{$\text{Learning rate } \alpha > 0, \text{ rank } k \in \{0, \dots, d\}, \text{ prior precision } \eta > 0$}
					\myinput{$\text{Regularization } \gamma \ge 0, \text{ damping } \xi \ge 0, \text{ Number of MC samples } M \ge 1$}
					\myinput{$\beta_1, \beta_2 \in (0, 1]$}
					Set $\matr{m} = \matr{0}$

					$\matr{z}_m \leftarrow \matr{\mu} + N^{-1/2} \matr{\hat{B}}^{-T} \matr{\varepsilon}_m$, where $\matr{\varepsilon}_m \sim \Normal(\matr{0}, \matr{I}_{d\times d}), \quad \forall m \in \{1, \dots, M\}$
					...
					$\matr{\mu} \leftarrow \matr{\mu} - \alpha \matr{\bar{S}}^{-1} \matr{\bar{m}}, \quad \text{ where } \matr{\bar{S}} = \matr{\hat{B}} \matr{\hat{B}}^{T} / (1 - \beta_2^t) + \xi \matr{I}$
