### Reliability/Calibration Code by https://github.com/hollance/reliability-diagrams
...
def _reliability_diagram_combined(bin_data,
                                  draw_ece, draw_mce, draw_bin_importance, draw_averages,
...
def _uncertainty_diagram_subplot(ax, bin_data,
    ...
    error_plt = ax.bar(positions, errors, bottom=0, width=widths, linewidth=1,
                     edgecolor=blue, color=blue, label="Error")
    ...
    ax.legend(handles=[error_plt, gap_plt], loc='upper left')


def _uncertainty_histogram_subplot(ax, bin_data,
                                  draw_averages=True,
                                  title="Examples per bin",
                                  xlabel="Uncertainty",
                                  ylabel="\% of Samples"):
    ...
    if draw_averages:
        error_plt = ax.axvline(x=bin_data["avg_error"], ls="solid", lw=3,
                             c="black", label="Error")
        uncert_plt = ax.axvline(x=bin_data["avg_uncertainty"], ls="dotted", lw=3,
                              c="#444", label="Avg. uncertainties")
        ax.legend(handles=[error_plt, uncert_plt])


def _uncertainty_diagram_combined(bin_data,
                                  draw_uce, draw_muce, draw_bin_importance, draw_averages,
                                  title, figsize, dpi, return_fig):
    ...
    _reliability_diagram_subplot(ax[0], bin_data, draw_uce, draw_muce, draw_bin_importance,
                                 title=title, xlabel="")
